# ============================================================
#  pipeline.py — Factual Error Detection Pipeline
#  Model: HuggingFace Inference API (langchain-huggingface)
#  Usage:
#    python pipeline.py \
#      --games   games.csv \
...
# ── CLI Arguments ─────────────────────────────────────────────
arg_parser = argparse.ArgumentParser(description="Factual Error Detection Pipeline")
arg_parser.add_argument("--games",   type=str, required=True,         help="Path to games.csv")
arg_parser.add_argument("--jsonl",   type=str, required=True,         help="Path to shared_task.jsonl")
arg_parser.add_argument("--model",   type=str, default="llama_small", help="Model key (default: llama_small)")
arg_parser.add_argument("--rows",    type=int, default=20,            help="Number of rows to process (default: 20)")
arg_parser.add_argument("--api_key", type=str, required=True,         help="HuggingFace API token")
args = arg_parser.parse_args()

# ── Config ────────────────────────────────────────────────────
HF_TOKEN = args.api_key
...
MODELS = {
    ...
    "qwen_medium":  "Qwen/Qwen2.5-72B-Instruct",
}
...
def prompt_fn(text_id, story, game_data, model_key):
...
# ── Load Data ─────────────────────────────────────────────────
print(f"Loading data from '{args.games}' and '{args.jsonl}' ...")
...
print(f"\nRunning pipeline with model : {model_key} ({MODELS[model_key]})")
print("-" * 60)
...
for i, row in games_df.head(args.rows).iterrows():
    ...
    try:
        parsed  = prompt_fn(text_id, story, game_data, model_key)
...
out_file = f"results_{model_key}.csv"
df_result.to_csv(out_file, index=False)

print("-" * 60)
print(f"Done in {elapsed}s — {len(df_result)} annotations saved to '{out_file}'")
