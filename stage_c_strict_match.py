...
Input  : formatted/stage_b/results_<run_id>_ab.csv
Output : formatted/stage_c/results_<run_id>_abc.csv
"""
...
def process_file(input_path, output_path, texts):
    ...
    for idx, row in df.iterrows():
        ...
        # Get actual tokens at the recorded span
    ...
    return total, populated, kept, blanked


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--input_dir", default="formatted/stage_b")
    parser.add_argument("--output_dir", default="formatted/stage_c")
    ...
    for fname in files:
        ...
        out_name = fname.replace("_ab.csv", "_abc.csv")
        out_path = os.path.join(args.output_dir, out_name)
        try:
            total, populated, kept, blanked = process_file(in_path, out_path, texts)
            print(f"  {fname:55s} -> {out_name}  ({populated}/{total} populated, kept={kept}, blanked={blanked})")
